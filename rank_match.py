...
#         similarity_score = cv_doc.similarity(jd_doc)
#         ranked_cvs.append((idx, similarity_score))

#     ranked_cvs = sorted(ranked_cvs, key=lambda x: x[1], reverse=True)
#     return ranked_cvs

# def rank_cvs_with_spacy_similarity(job_description, cv_ner_outputs_list):
#     nlp = spacy.load('en_core_web_lg')
...
#     ranked_cvs = []
#     for idx, cv in enumerate(cv_ner_outputs_list):
#         cv_text = ' '.join([entity_text for entity_text, _ in cv])  # Join only entity text
#         cv_doc = nlp(cv_text)
